  ...
  describe '#call' do
    context '存在するルームIDと有効なタイトルが渡された場合' do
      it 'トピックを作成して成功レスポンスを返す' do
        room = create(:room)
        result = AddTopicService.call(room_id: room.id, title: 'Test Topic')
        ...
        expect(result.topic.status).to eq('voting')
    ...
    context '存在しないルームIDが渡された場合' do
      it 'エラーメッセージを含む失敗レスポンスを返す' do
        result = AddTopicService.call(room_id: SecureRandom.uuid, title: 'Test Topic')
    ...
    context '無効なタイトルが渡された場合' do
      it 'エラーメッセージを含む失敗レスポンスを返す' do
        room = create(:room)
        result = AddTopicService.call(room_id: room.id, title: '')
