class MergeTopicsService
  ...
  def call(source_topic_id:, target_topic_id:)
    ...
    return { success: false, topic: nil, errors: ["統合元のトピックが見つかりません"] } unless source_topic
    return { success: false, topic: nil, errors: ["統合先のトピックが見つかりません"] } unless target_topic

    unless source_topic.status == "organizing" && target_topic.status == "organizing"
      return { success: false, topic: nil, errors: ["整理フェーズのトピックのみ統合できます"] }
    end

    unless source_topic.room_id == target_topic.room_id
      return { success: false, topic: nil, errors: ["同じルームのトピックのみ統合できます"] }
    end
    ...
    if target_topic.update(title: merged_title, description: merged_description)
      ...
      { success: true, topic: target_topic, errors: [] }
    else
      { success: false, topic: target_topic, errors: target_topic.errors.full_messages }
    end
  end
end
