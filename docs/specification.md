...
- **目的**: Googleアカウントで認証するユーザー
- **属性**:
  - `id`: UUID（主キー）
  - `email`: メールアドレス（必須、一意）
  - `name`: ユーザー名（必須）
  - `google_uid`: GoogleアカウントのUID（必須、一意）
  - `avatar_url`: アバター画像URL（オプション）
  - `created_at`, `updated_at`: タイムスタンプ
- **関連**:
  - 現時点では直接的な関連はなし（将来的にルーム作成者や管理者との関連を追加予定）
- **ビジネスロジック**:
  - Google OAuth認証時に自動的に作成または更新
  - `google_uid`で一意に識別
...
- Googleアカウントを使用したOAuth 2.0認証を実装
- JWT（JSON Web Token）を使用したセッション管理
- GraphQLリクエストにJWTトークンをAuthorizationヘッダーで送信
- 認証が必要な操作は`current_user`をコンテキストから取得（現時点では一部のmutationでTODOコメントあり）
